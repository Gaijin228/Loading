    ...
    <section class="p-4 md:flex md:justify-center gap-8">
        ...
        <div>
            ...
            <div>
                ...
                <p class="mt-2 mb-4">
                    <form action="{{ route('cart_add', $produit->id ) }}" id="panier_add" method="POST" enctype="multipart/form-data">
                        @csrf
                        <select name="size" id="size-choice" class="w-full h-10 border border-double border-slate-300 rounded-md hover:border-cyan-600">
        ...
        <div class="mx-14 mb-14 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            @foreach ($produit->recommandations as $produit_recommande)
                <div class="max-w-sm border border-double shadow-lg rounded-lg bg-white">
                    <img src="{{ asset('img/' . $produit_recommande->photo_principale) }}" alt="{{ $produit_recommande->nom }}" class="rounded-t-lg">
                    <div class="mt-4 px-4">
                        <h5 class="text-xl text-gray-600 font-bold">Style Pack</h5>
                        <span>{{ $produit_recommande->description }}</span><br>
                        ...
                        <div class="mt-3 flex justify-between mb-3">
                            <span class="text-lg text-blue-700 font-bold">{{ number_format($produit_recommande->prix_ht, 2) }} €</span>
                            <a href="{{ route('voir_produit', $produit_recommande->id) }}" target="_blank" class="bg-yellow-400 hover:bg-cyan-700 rounded-md px-1">
